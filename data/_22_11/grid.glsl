...
uniform float smoothstepEdgeA;
uniform float smoothstepEdgeB;
...
void main() {
    ...
    if(sdfCropEnabled){
        ...
        sdfAlpha = smoothstep(smoothstepEdgeA, smoothstepEdgeB, sdfResult);
    }
    gl_FragColor = vec4(mix(bgColor.rgb, fgColor.rgb, min(alpha, sdfAlpha)), 1.);
}